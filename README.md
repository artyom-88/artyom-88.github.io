...
The blocklist lives in `compromised.txt`. The normal format is one exact `package@version` per line. Comments and empty lines are ignored. The checker also understands package-only entries and would treat them as "block every version of this package", but the repository policy is to keep the file exact and project-scoped so the list stays actionable and does not over-block unrelated versions. The updater also maintains comment metadata in the same file: `# refreshed-at:` records the last successful advisory refresh timestamp, and `# manual-entry:` marks exact emergency blocks that should survive future refreshes until they are removed explicitly.
