...
import { initialState } from 'features/career/career.constants';
import career, { actions } from 'features/career/career.slice';
...
describe('reducers', () => {
  describe('career', () => {
    ...
    it('should return correct career load success state', () => {
      const action: PayloadAction<CareerModel[]> = actions.loadListSuccess(CAREER_ITEMS_MOCK);
      const state: CareerState = career(initialState, action);
