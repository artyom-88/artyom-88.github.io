...
import { default as careerSaga, loadCareerListSaga } from 'features/career/career.saga';
  ...
  describe('career', () => {
    it('loadCareerList', () => {
      ...
      testSaga(loadCareerListSaga)
        ...
        .put(actions.loadListSuccess(CAREER_ITEMS_MOCK))
        .next()
        .isDone();
    });

    it('careerSaga', () => {
      testSaga(careerSaga).next().takeEvery(actions.loadList.type, loadCareerListSaga).next().isDone();
    });
  });
});
